...
def run_terraform_apply():
    terraform_directory = "/home/ubuntu/terraform"
    subprocess.run(["terraform", "apply", "-auto-approve"], cwd=terraform_directory, check=True)
 
def run_delete_non_ready_nodes():
    subprocess.run(["kubectl", "delete", "nodes", "$(kubectl get nodes | grep NotReady | cut -f 1 -d ' ')"], check=True)

def main(worker_to_add=0):
    update_terraform_variable(worker_to_add)

    run_terraform_apply()
    run_delete_non_ready_nodes()
